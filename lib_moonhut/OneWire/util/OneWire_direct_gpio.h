...
// --- MoonHut patch: correct the GPIO range on ESP32-S2/S3 ------------------
// Upstream 2.3.8 hard-codes the ORIGINAL ESP32's GPIO map: reads and writes are
// gated on `pin < 46`, and output-enable on `pin <= 33`, because that chip tops
// out at GPIO39 and its pins 34-39 are input-only. The S2/S3 have NO input-only
// pins and their GPIO space runs to 46/48.
//
// So on an S3 any pin above 33 silently never becomes an output, and any pin
// above 45 reads as a constant 0. OneWire::reset() then spins out all 125 of
// its retries waiting for the bus to read high and returns 0 - "no presence
// pulse" - without ever having driven the line. Every probe on such a pin looks
// dead no matter how well it is wired. (Cost us three probe batches and a
// vendor change before we read this file; see devices/vision-master-e290/.)
//
// The bounds below restore upstream behaviour exactly on the original ESP32
// and make S2/S3 pins work. Nothing else in the library is modified.
#ifndef SOC_GPIO_PIN_COUNT
#define SOC_GPIO_PIN_COUNT 46
#endif
#if CONFIG_IDF_TARGET_ESP32
#define ONEWIRE_CAN_OUTPUT(pin) ((pin) <= 33) // GPIO34-39 are input-only
#else
#define ONEWIRE_CAN_OUTPUT(pin) (1)           // S2/S3/C3: every valid GPIO drives
#endif
// ---------------------------------------------------------------------------
...
#else
    if ( digitalPinIsValid(pin) && ONEWIRE_CAN_OUTPUT(pin) )
    {
#if ESP_IDF_VERSION_MAJOR < 4      // IDF 3.x ESP32/PICO-D4
        uint32_t rtc_reg(rtc_gpio_desc[pin].reg);
        ...
        else // GPIO32+ live in the second register bank
